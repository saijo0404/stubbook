...
export const SALE_KEYWORD_REGEX =
  /(?:售票時間|售票日期|售票日程|售票日|售票時程|售票開放|開放售票|售票資訊|開始售票|開賣時間|開賣日期|開賣日程|開賣日|正式開賣|全面開賣|啟售時間|啟售日期|啟售日程|啟售日|一般啟售|會員啟售|卡友啟售|優先啟售|正式啟售|開售時間|開售日期|開售日程|開售日|正式開售|全面開售|一般開售|開售|購票時間|購票日期|購票日程|購票日|開放購票|開始購票|預售時間|預售日期|預售日|預售開賣|預售|現場售票|現場開賣|現場票|預購時間|預購日期|會員預購|卡友優先|卡友預售|卡友購票|優先預購|優先購票|優先售票|先行預約|先行販售|登記抽票|抽選登記|抽票登記|實名制抽票|抽票|抽選|搶票時間|開票時間|公售時間|一般售票|會員優先|早鳥會員|早鳥優先|清票時間|釋票時間|清票|釋票|二次開賣|加開開賣|二次售票|開始販售|販售時間|販售日期|ticket\s*sale|on\s*sale|general\s*sale|presale|public\s*sale|sale\s*start|sale\s*date|sale\s*time|ticketing)/i;
...
export function extractSalePhasesFromContent(
  ...
  const lines = normalized
    ...
    .flatMap((seg) => {
      if (
        /[，,]\s*(?=[^，,]*?(?:售票|開賣|啟售|開售|販售|預售|預購|抽票|一般|優先|全面|現場|早鳥|會員))/i.test(
          seg
        )
      ) {
        return seg
  ...
  for (const line of lines) {
    ...
    let cleanedLine = line
      ...
      // 2. 移除前置售票關鍵字標籤
      .replace(
        /^[\[【〔［]?(?:售票時間|開賣時間|啟售時間|開售時間|預售時間|優先購票|會員預購|登記抽票|全面開賣|全面開售|正式開售|搶票時間|開賣日程|開售日程|售票日程|售票日|開賣日|開售日|開票時間|公售時間|一般售票|一般開售|卡友優先|會員優先|正式開賣|清票時間|釋票時間|開始販售|販售時間|販售日期|購票時間|購票日期|啟售日期|開賣日期|開售日期|一般啟售|優先啟售|會員啟售|卡友啟售|ticket\s*sale|on\s*sale|general\s*sale|presale|public\s*sale|sale\s*start|sale\s*time|ticketing)[\]】〕］]?(?:時間)?[:：\s]*/i,
        ''
      )
      // 3. 移除日期與時間本體
      .replace(
