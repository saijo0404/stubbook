...
- [x] **ibon 售票系統適配器**
  - [x] 支援 7-ELEVEN ibon 售票網頁結構解析（多場次、全區票價級距、售票時間）
  - [x] 解析實體機台取票與線上電子票規則
- [x] **FamiTicket 全網購票網適配器**
  - [x] 支援全家便利商店 FamiTicket 系統解析與票圖/分區抓取
  - [x] 應對其多層級選位網址結構
- [x] **寬宏售票 (Kham Ticketing) 適配器**
  - [x] 支援寬宏售票系統之大型展演、藝文舞台劇與海外巡演解析
- [x] **INDIEVOX 獨立音樂網適配器**
  - [x] 支援獨立音樂 Livehouse（如 Legacy、THE WALL、PIPE 等）小型專場與預售票結構
