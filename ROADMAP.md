...
│  智慧行事曆  │     │ 資料備份還原 │     │ 售票平台擴展 │     │ 串流音樂生態 │     │ 原生雙端發行 │
│  與搶票提醒  │     │ 與冷熱遷移   │     │ (ibon/全網等)│     │(Spotify/Apple│     │(Android/iOS) │
│   (v1.1.0)   │     │   (v1.2.0)   │     │   (v1.3.0)   │     │ /YT Music)   │     │   (v2.0.0)   │
└──────────────┘     └──────────────┘     └──────────────┘     └──────────────┘     └──────────────┘
```
...
- [ ] **智慧日期萃取與排程標記**
  - [ ] 爬蟲管線擴充：解析售票系統之「會員優先購票」、「實名制登記/抽票」、「一般公開售票」及演出場次起訖時間
  - [ ] 自動標記活動關鍵時間節點（開賣搶票倒數、整票釋票日、實名核對日、取票日、演出日）
- [ ] **視覺化美觀行事曆儀表板 (In-App Live Calendar)**
  - [ ] 手帳風格月曆/週曆視圖：高顏值霓虹/膠卷風格標籤，直觀區分「購票日 🏷️」與「演出日 🎫」
  - [ ] 搶票倒數雷達卡片：依時間遠近排序即將開賣的活動，支援開賣前 10 分鐘 / 1 小時視覺警示與高亮
- [ ] **跨平台日曆同步與匯出**
  - [ ] 一鍵匯出標準 `.ics` (iCalendar) 檔案（相容 iOS Calendar、Google 日曆、Outlook）
  - [ ] 支援 Google Calendar 與 Apple Calendar 快速加入 URL 生成
  - [ ] 行動端系統級日曆通知與搶票前推播提醒整合
...
- [ ] **全量備份機制 (`.stubbook` 容器封裝)**
  - [ ] 一鍵打包本地 SQLite 資料庫快照與本機上傳之多媒體檔案（票根、相片、影片、視角圖庫）
  - [ ] 支援 ZIP 壓縮封裝與 AES-GCM 本地可選密碼保護，產出 `.stubbook` 專屬封裝檔案
  - [ ] 檔案完整性校驗：加入 SHA-256 數位簽章防損毀校驗
- [ ] **結構化資料匯出與還原**
  - [ ] JSON 標準資料集匯出（活動、場次、歌手、手帳、記帳周邊、歌單）
  - [ ] 智慧還原與衝突策略：支援「覆蓋 (Overwrite)」、「保留並合併 (Merge)」、「僅更新差異 (Diff)」模式
  - [ ] 遷移導航精靈：提供還原前資料預覽、統計比對與錯誤回滾 (Rollback) 機制
...
| `v1.1.0` | 📋 規劃中   | 智慧行事曆排程、搶票倒數雷達、.ics 匯出與美觀日曆儀表板   |
| `v1.2.0` | 📋 規劃中   | 本地資料安全、.stubbook 全量封裝備份、JSON 匯入匯出與還原  |
