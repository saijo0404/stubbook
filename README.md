...
### 12. 📊 推活熱量大數據、全出費分析與潮流社群卡工廠 (Passion Analytics & Aesthetic Card Generator)

- **推活全量出費大數據與遠征支出佔比**：
  - 全旅程 6 大消費維度（門票、高鐵/台鐵/機票交通、住宿、官方周邊、應援餐飲、其他雜項），自動整合票券與周邊花費。
  - 出費結構圓形進度條、年度場均消費與遠征出費佔比 (`farExpeditionRate`) 自動計算。
- **365 日參戰熱力矩陣與狂熱指標儀表板**：
  - 類 GitHub Commit 綠格子的 365 日全年參戰熱力圖（0~4 級綠色發光梯級），支援互動 Tooltip 檢視當日場次。
  - 星期狂熱分析（自動標定最常參戰之熱血星期，如週六狂熱）、月度參戰趨勢統計。
  - 現場聽過真實歌曲不重複解鎖總數累計與神曲排行榜 Top 5。
  - 推活熱量指數 (Passion Score) 與專屬推活稱號頭銜（如「傳說級狂熱推活大師 Lv.99」）。
- **潮流社群分享卡工廠 (Aesthetic SNS Card Generator)**：
  - 🧾 **文青熱感應收據風格 (Vintage Receipt Card)**：復古感熱紙鋸齒撕邊、詳細出費清單、Setlist 曲目與防偽驗證條碼。
  - 💿 **CD 壓克力寶石外殼風格 (Acrylic Jewel Case)**：高光澤壓克力外殼、全像雷射貼紙、光碟齒輪與巡演專屬編號。
  - 🖼️ **IG Stories 透明背景貼紙 (Transparent PNG Sticker)**：全透明背景票根貼紙，支援無縫貼在 Instagram / Threads 限時動態照片上。
  - 🎨 **4 大外觀調色盤與浮水印**：黑曜暗黑、拍立得白、霓虹紫、復古牛皮紙，支援自訂社群帳號浮水印與 2x 高清 PNG 輸出/原生 Web Share 分享。

### 13. 🤝 四級隱私好友圈、同行 P2P 票根互傳與讓換票核驗 (Social Connect & P2P Stub Sync)

- **四級隱私權限與同行參戰夥伴標記 (Privacy Levels & Concert Buddies)**：
  - 手帳支援 `PUBLIC`（公開）、`FRIENDS`（好友可見）、`CLOSE_FRIENDS`（僅摯友圈）、`PRIVATE`（私密）四層權限控制，預設最高隱私規格。
  - 本地好友通訊錄與階級分類（死黨、好友、點頭之交），標記同行夥伴、同行身分與鄰近座位，自動統計彼此累計參戰場次。
- **去中心化現場 P2P 票根快傳 (P2P AirDrop & QR Share)**：
  - 專屬 `stubbook-p2p://` 去中心化協議，免經第三方伺服器，現場掃描動態 QR Code 即可一秒傳遞演出、場館、Setlist 歌單與公開心得。
  - 自動剔除票價、條碼序號與個資，接收端一鍵入庫並自動建立同行夥伴關聯。
- **讓換票流轉追蹤與官方防偽核驗助手 (Safe Ticket Exchange Companion)**：
  - 讓票、換票、求票五階段流轉追蹤（洽談中 ➔ 定金 ➔ 面交驗票 ➔ 取票 ➔ 完成）。
  - 拓元售票 (tixCraft)、KKTIX、ibon、全家 FamiTicket 四大官方防偽特徵庫對照、互動式防偽驗收 Checklist 與防詐安全對話範本一鍵複製。
